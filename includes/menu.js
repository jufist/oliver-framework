...
m.useNodeCMD = async (cmd) => {
	...
	argv.shift();
	argv.shift();
	var cmdc = sprintf(cmd, extend({"arguments": '"' + argv.join('" "') + '"'}, argv));
	var cmdcd = cmdc.split(/\n/).filter(n => n.trim()).join(" && ");
	cmdcd = cmdcd.replace(/\\/g, "\\\\")
					 .replace(/\$/g, "\\$")
					 .replace(/'/g, "\\'")
					 .replace(/"/g, "\\\"");
	cmdcd=`bash -c "${cmdcd}"`;
		...
		ret = async () => {
			...
			try {
				if (newcallback[6] == 'node-cmd') {
				 	out = await m.useNodeCMD(`>/dev/null cd ${menuitem['callback_location']}; ` + newcallback[0]);
					// cl(out.message);

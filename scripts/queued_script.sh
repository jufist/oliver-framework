...
if ! flock -n "${lock_fd}"; then
    ...
    echo "[$$] Another instance is running at ${lock_fd} Arg=${LOCKFILE} Pos=$cur_queue_pos. Waiting for the lock in $timeout seconds with queue $QUEUEFILE. You might try to clear related files at ${LOCKSFILE} if needed" >&2
